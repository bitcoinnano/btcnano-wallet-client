...
var BASE_URL = 'http://114.67.37.2:10568/bws/api';
...
API.prototype.seedFromRandom = function(opts) {
  ...
  opts = opts || {};
  this.credentials = Credentials.create(opts.network || 'main');
};
...
console.log('hardcodedOk = '+ hardcodedOk);
...
API.prototype.seedFromRandomWithMnemonic = function(opts) {
  ...
  opts = opts || {};
  this.credentials = Credentials.createWithMnemonic(opts.network || 'main', opts.passphrase, opts.language || 'en', opts.account || 0);
};
...
API.prototype.seedFromMnemonic = function(words, opts) {
  ...
  opts = opts || {};
  this.credentials = Credentials.fromMnemonic(opts.network || 'main', words, opts.passphrase, opts.account || 0, opts.derivationStrategy || Constants.DERIVATION_STRATEGIES.BIP44, opts);
};
...
API.prototype.importFromMnemonic = function(words, opts, cb) {
  ...
  function derive(nonCompliantDerivation) {
    return Credentials.fromMnemonic(opts.network || 'main', words, opts.passphrase, opts.account || 0, opts.derivationStrategy || Constants.DERIVATION_STRATEGIES.BIP44, opts,{
...
API._buildSecret = function(walletId, walletPrivKey,network) {
  ...
  return _.padRight(widBase58, 22, '0') + walletPrivKey.toWIF() + (network == 'testnet' ? 'T' : 'L') + 'Nano';
};
...
coin: 'dash',
      network: networkChar == 'T' ? 'testnet' : 'main',
    };
  } catch (ex) {
...
API.prototype.getFeeLevels = function(network, cb) {
  ...
  $.checkArgument(network || _.includes(['main', 'testnet'], network));

  self._doGetRequest('/v2/feelevels/?coin=Nano' + '&network=' + (network || 'main'), function(err, result) {
    if (err) return cb(err);
...
var coin = 'Nano';
  if (!_.includes(['Nano'], coin)) return cb(new Error('Invalid coin'));

  var network = opts.network || 'main';
  if (!_.includes(['testnet', 'main'], network)) return cb(new Error('Invalid network'));
...
API.prototype.getBalance = function(opts, cb) {
  ...
  if (opts.coin) {  
  if (!_.includes(['Nano'], opts.coin)) return cb(new Error('Invalid coin'));
...
API.signTxProposalFromAirGapped = function(key, txp, unencryptedPkr, m, n, opts) {
 ...
 if (!_.includes(['Nano'], coin)) return cb(new Error('Invalid coin'));
  ...
  if (key.slice(0, 4) === 'xprv' || key.slice(0, 4) === 'tprv') {
    ...
    if (key.slice(0, 4) === 'tprv' && txp.network == 'main') throw new Error("main HD keys must start with xprv");
